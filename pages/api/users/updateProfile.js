...
export default async function handler(req, res) {
  if (req.method === 'PATCH') {
    ...
    const isChangingPassword = oldPassword.length > 0 && newPassword.length > 0; // if true we add password property to user object
    const user = {}; // it will contain the updated user data.
    if (isUpdatingEmail) { // if the email field is changed  check for existing one 
